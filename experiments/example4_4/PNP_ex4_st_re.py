...
def get_args(): 
    ...
    parser.add_argument('-r_c3', '--range3', default=1, help='Range of initial weights', type=float)
    parser.add_argument('-r_phi', '--range4', default=1, help='Range of initial weights', type=float)
...
def main(r_c1, r_c2, r_c3, r_phi, n, Nt, Nre, n_int, m, tt, p1, device): 
    a = 0
    b = 1
    c = 0
    d = 1
    ...
    # RaNN
    # c3m = RNN_funs.rnn_gauss_exact_1d(3, m, 1).to(device)
    # SO-RaNN
    # c3m = RNN_funs.rnn_gauss_positive_exact_1d(3, m, 1).to(device)
    c3m = RNN_funs.rnn_gauss_positive1_exact_1d(3, m, 1).to(device)
    RNN_funs.weights_init_uniform_0(c3m, -r_c3, r_c3)
    ...
    PNP_assemble_st_re_ex4.pnp_ex4_picard_st_remass_G_plus(n, n_int, Nt, Nre, s, x2, wr2, m, c1m, c2m, c3m, phim, a, b, c, d, t1, D1, D2, D3, z1, z2, z3, eps, threshold, tt, p1)

if __name__ == '__main__':
    args = get_args() 
    # device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    device = torch.device('cpu')
    main(r_c1=args.range1, r_c2=args.range2, r_c3=args.range3, r_phi=args.range4, n=args.numbers1, Nt=args.numbers_t, Nre=args.numbers_re,
